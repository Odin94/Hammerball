...
bool isHost = true; //true if you dont play MP - used to check whether this entity has to calculate game logic
...
void setupServer(string ip) //doesnt setup clientsocket, since clientsocket is opened on rnd available port (0) anyways
{
    serverSocket = SDLNet_UDP_Open(8133);
        
    SDLNet_ResolveHost(&UDPaddress, ip.c_str(), 8134);//TODO change port for every client..?
...
{
    int port = 8132 + playernumber; //playernumber for clients is between 2 and 4 -> ports are 8134 - 8136
...
//setE: xywh, dmgfr, dmgfrdmg, img, maxframes, time2nextframe, traversable, active, minTime, maxTime, TimeTillReset
void initStage()
{
    ...
    for(int i = 0; i < 25;i++)
    {
        for(int j = 0; j < 15; j++)
        {
            ...
            ATiles[i][j].setE(i*66, j*66, 66, 66, mpc[0], mpc[1], mpc[2], mpc[3], mpc[4], mpc[5]!= 0, mpc[6]!=0, mpc[7], mpc[8], mpc[9]);
            ...
            BTiles[i][j].setE(i*66, j*66, 66, 66, -1, 0, -1, 0, 1, true, true, 0, 0, 0);
    ...
    ATiles[11][12].setE(11*66, 12*66, 66, 66, 8, 1, FALLING_TILE, 9, 100, true, true, 2000, 3000, 2500);
    ATiles[11][12].generateTime();
    ATiles[11][12].clip_rect.x = 0;
  
    ATiles[11][11].setE(11*66, 11*66, 66, 66, 8, 1, FALLING_TILE, 9, 250, true, true, 2000, 3000, 2500);
    ...
    else
    {
        for(int i = 0; i < 25;i++)
        {
            for(int j = 0; j < 15; j++)
            {
                ...
                {
                    apply_surface(ATiles[i][j].x, ATiles[i][j].y, surfaces[ATiles[i][j].img], screen, &ATiles[i][j].clip_rect);
                }
                if(ATiles[i][j].needsBorder) //check if borders from top, left, right or bottom need to be drawn
                {
                    if(!ATiles[i-1][j].needsBorder) 
                    {
                        apply_surface(ATiles[i][j].x, ATiles[i][j].y, Borders, screen, 0, 0, 66, 66);
                    }
                    if(!ATiles[i+1][j].needsBorder)
                    {
                        apply_surface(ATiles[i][j].x, ATiles[i][j].y, Borders, screen, 67, 0, 66, 66);
                    }
                    if(!ATiles[i][j-1].needsBorder)
                    {
                        apply_surface(ATiles[i][j].x, ATiles[i][j].y, Borders, screen, 134, 0, 66, 66);
                    }
                    if(!ATiles[i][j+1].needsBorder)
                    {
                        apply_surface(ATiles[i][j].x, ATiles[i][j].y, Borders, screen, 201, 0, 66, 66);
                    }
                }
                if(BTiles[i][j].active)
                {
                    apply_surface(BTiles[i][j].x, BTiles[i][j].y, surfaces[BTiles[i][j].img], screen, &BTiles[i][j].clip_rect);
                }
            }
        }
        ...
        if(player.alive)
        {
           apply_surface(player.x, player.y, PlayerSprite, screen, &player.clip_rect);
        }
        else
        {
        ...
        if(multiplayer || localmultiplayer)
        {
            if(player2.alive){apply_surface(player2.x, player2.y, PlayerSprite, screen, &player2.clip_rect);} else {apply_surface(player2.x, player2.y, PlayerDead, screen);}
            if(playercount > 2 && player3.alive)
            {
                apply_surface(player3.x, player3.y, PlayerSprite, screen, &player3.clip_rect);
                if(playercount > 3 && player4.alive)
                {
                    apply_surface(player4.x, player4.y, PlayerSprite, screen, &player4.clip_rect);
                }
            }
        }
...
void handleMP()
{
    if(isHost) //gets updates immediatly
    {
        ...
        if(SDLNet_UDP_Recv(serverSocket, recPac)>0)
        {
            ...
            if(temp[0][0] == '2')
            {
                ...
                if(!hitStackPreventor.is_started())
                {
                    ...
                    {
                        ball.getHit(player2);
                        hitball = '1'; //send hitball = 1 back to confirm that ball was hit TODO: prevent ball from getting hit twice due to packet loss("recently hit ball" variable dependant on timer?)
                ...
                if(hitStackPreventor.is_started() && hitStackPreventor.get_ticks() > 500)
                {
        ...
        sstream << player.x << "," << player.y << "," << ball.x << "," << ball.y << "," << islethal << "," << hitball <<"," << player2.x << "," << player2.y << "," <<player3.x << "," << player3.y << "\0";
        ...
        sendPac->len = strlen((char*)sendPac->data)+1; //length of data when cast to string; +1 for finish-character 
        SDLNet_UDP_Send(clientSocket, -1, sendPac);//sets packet->channel (socket,channel,packet)
        
        if(playernumber > 2)
        {
            sendPac->address.host = Client3Adr.host;
            sendPac->address.port = Client3Adr.port; //port# is wrong! its the TCP port number
    ...
    else
    {
        ...
        sstream << playernumber <<","<< player.x << "," << player.y << "," << hitball << "\0";
        string result = sstream.str();
...
void update(Uint32 delta)
{
    ...
    if(player.alive)
    {
        ...
        if(BTiles[(player.x+player.w/2) / 66][(player.y+player.h/2) / 66].damage > 0 || ATiles[(player.x+player.w/2) / 66][(player.y+player.h/2) / 66].damage > 0)
        {
            player.alive = false;
        }
    }
    ...
    if(upgrade.active)
    {
        if(abs(player.x + player.w/2 - (upgrade.x + upgrade.w/2)) < 58 && abs(player.y + player.h/2 - (upgrade.y + upgrade.h/2)) < 58)
        {
            ...
            if(upgrade.type == 's')
            {
                if(player.speedup < player.speedupcap){player.speedup++;};
            }
            if(upgrade.type == 'p')
            {
                if(player.powerup < player.powerupcap){player.powerup++;};
            }
        }
        if(multiplayer || localmultiplayer)
        {
            if(abs(player2.x + player2.w/2 - (upgrade.x + upgrade.w/2)) < 58 && abs(player2.y + player2.h/2 - (upgrade.y + upgrade.h/2)) < 58)
            {
                ...
                if(upgrade.type = 's')
                {
                    if(player2.speedup < player2.speedupcap){player2.speedup++;};
                }
                if(upgrade.type = 'p')
                {
                    if(player2.powerup < player2.powerupcap){player2.powerup++;};
                }
            }
            if(playercount > 2)
            {
                if(abs(player3.x + player3.w/2 - (upgrade.x + upgrade.w/2)) < 58 && abs(player3.y + player3.h/2 - (upgrade.y + upgrade.h/2)) < 58)
                {
                    ...
                    if(upgrade.type = 's')
                    {
                        if(player3.speedup < player3.speedupcap){player3.speedup++;};
                    }
                    if(upgrade.type = 'p')
                    {
                        if(player3.powerup < player3.powerupcap){player3.powerup++;};
                    }
                }
                if(playercount > 3)
                {
                    if(abs(player4.x + player4.w/2 - (upgrade.x + upgrade.w/2)) < 58 && abs(player4.y + player4.h/2 - (upgrade.y + upgrade.h/2)) < 58)
                    {
                        ...
                        if(upgrade.type = 's')
                        {
                            if(player4.speedup < player4.speedupcap){player4.speedup++;};
                        }
                        if(upgrade.type = 'p')
                        {
                            if(player4.powerup < player4.powerupcap){player4.powerup++;};
                        }
                    }
                }
            }
        ...
        while( SDL_PollEvent( &event ) )
        {
            ...
            {   
                if(!inputdone)
                {
                    ...
                    if(event.key.keysym.sym == SDLK_ESCAPE){running=false;inTitle=false;}
                    if(event.key.keysym.sym == SDLK_RETURN)
                    {
                        ...
                        else if(inputcounter == 1)
                        {
                            ...
                            {
                                SDLNet_ResolveHost(&address, StrIn.str.c_str(), 8134); connectionIP = StrIn.str;
                                if(StrIn.str == "local")
                                {
                ...
                {
                    case SDLK_RETURN: inTitle = false; UpgradeSpawner.start(); break;
                    case SDLK_ESCAPE: if(multiplayer){multiplayer=false;isHost=true; SDLNet_UDP_Close(serverSocket);}else{running = false; inTitle = false;}break;
                    case SDLK_c: screen = SDL_SetVideoMode( SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_BPP, SDL_SWSURFACE | SDL_RESIZABLE | SDL_FULLSCREEN );break;
                    case SDLK_v: screen = SDL_SetVideoMode( SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_BPP, SDL_SWSURFACE | SDL_RESIZABLE );break;  

                    case SDLK_h: isHost = true; multiplayer = true; setupServer(connectionIP); break;
                    case SDLK_j: isHost = false; multiplayer = true; setupClient(connectionIP); draw(); break;
                }
            }
        }

        if(multiplayer && !TCPConnectionDone) //TODO: Multithread dis shiz (or set nonblocking..?)
        {
            if(isHost)
            {
                TCPClient = SDLNet_TCP_Accept(TCPServer);
                    if(TCPClient)
                    {
                        ...
                        //cout << toDottedIP(SDLNet_TCP_GetPeerAddress(TCPClient)->host);
                        SDLNet_ResolveHost(&Client2Adr, toDottedIP(SDLNet_TCP_GetPeerAddress(TCPClient)->host).c_str(), 8134);
                        SDLNet_TCP_Close(TCPClient);

                            if(playercount > 2)
                            {
                                SDLNet_ResolveHost(&Client3Adr, toDottedIP(SDLNet_TCP_GetPeerAddress(TCPClient2)->host).c_str(), 8135);
                                SDLNet_TCP_Close(TCPClient2);

                                if(playercount > 3)
                                {
                                    SDLNet_ResolveHost(&Client4Adr, toDottedIP(SDLNet_TCP_GetPeerAddress(TCPClient3)->host).c_str(), 8136);
    ...
    while(running)
    {
        ...
        while( SDL_PollEvent( &event ) )
        {
            ...
            {   
                switch( event.key.keysym.sym )
                {
                    case SDLK_w:                                                    if(localmultiplayer){player2.velcapy = -(10 + player2.speedup); break;} 
                    case SDLK_UP: player.velcapy = -(10 + player.speedup); break;
                    case SDLK_s:                                                    if(localmultiplayer){player2.velcapy = 10 + player2.speedup; break;}
                    case SDLK_DOWN: player.velcapy = (10 + player.speedup); break;
                    case SDLK_a:                                                    if(localmultiplayer){player2.velcapx = -(10 + player2.speedup); break;}
                    case SDLK_LEFT: player.velcapx = -(10 + player.speedup); break;
                    case SDLK_d:                                                    if(localmultiplayer){player2.velcapx = (10 + player2.speedup); break;}
                    case SDLK_RIGHT: player.velcapx = (10 + player.speedup); break;

                    case SDLK_SPACE: if(localmultiplayer){
                            if(!clickspamprevent0r2.started)
                            {
                                clickspamprevent0r2.start();
                            }

                            if(abs(player2.x+player2.w/2 - (ball.x+ball.w/2)) <= 120 && abs(player2.y+player2.h/2 - (ball.y+ball.h/2)) <= 120 && (clickspamprevent0r2.get_ticks() == 0 || clickspamprevent0r2.get_ticks() > 500)) //half a second cd
                            {
                                    ball.getHit(player2);
                            }
                        } break;
                    ...
                    case SDLK_r:if(localmultiplayer){player.set(100,270,66,66,0,0); player.alive = true; player2.set(500,270,66,66,0,0); player2.alive = true; ball.set(500,500,66,66,0,0);}
                            else if(!multiplayer){player.set(100,270,66,66,0,0); player.alive = true; ai.set(500,270,66,66,0,0); ai.alive = true; ball.set(500,500,66,66,0,0);} break;
            ...
            if( event.type == SDL_KEYUP )
            {
                switch( event.key.keysym.sym )
                {
                    case SDLK_w:                                                    if(localmultiplayer){if(player2.velcapy < 0){player2.velcapy = 0;} break;}
                    case SDLK_UP: if(player.velcapy < 0){player.velcapy = 0;} break;
                    case SDLK_s:                                                    if(localmultiplayer){if(player2.velcapy > 0){player2.velcapy = 0;} break;}
                    case SDLK_DOWN: if(player.velcapy > 0){player.velcapy = 0;} break;
                    case SDLK_a:                                                    if(localmultiplayer){if(player2.velcapx < 0){player2.velcapx = 0;} break;}
                    case SDLK_LEFT: if(player.velcapx < 0){player.velcapx = 0;} break;
                    case SDLK_d:                                                    if(localmultiplayer){if(player2.velcapx > 0){player2.velcapx = 0;} break;}
                    case SDLK_RIGHT: if(player.velcapx > 0){player.velcapx = 0;} break;
                }
            }
            ...
            if( event.type == SDL_MOUSEBUTTONDOWN )
            {
                ...
                if( event.button.button == SDL_BUTTON_LEFT )
                {
                    ...
                    if(abs(player.x+player.w/2 - (ball.x+ball.w/2)) <= 120 && abs(player.y+player.h/2 - (ball.y+ball.h/2)) <= 120 && (clickspamprevent0r.get_ticks() == 0 || clickspamprevent0r.get_ticks() > 500)) //half a second cd
                    {
                        if(isHost)
                        {
                            ball.getHit(player);
