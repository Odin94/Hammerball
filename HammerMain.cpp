...
void draw()
{
    ...
    else
    {
        ...
        if(multiplayer || localmultiplayer)
        {
            if(player2.alive){apply_surface(drawscale, player2.x, player2.y, PlayerSprite, screen, &player2.clip_rect);} else {apply_surface(drawscale, player2.x, player2.y, PlayerDead, screen);}
...
void handleMP() //TCP introduction makes both sides crash when moving the window (and also randomly when connected to a different PC, recv failure?)
{
    //This is just to use socketready later and should probably be done once upon initialization and never again
...
*/
    if(isHost)
    {
      /*  int active = SDLNet_CheckSockets(set, 1);         
        ...
        sstream << player.x << "," << player.y << "," << ball.x << "," << ball.y << "," << islethal << "," << hitball <<"," << player2.x << "," << player2.y << "," <<player3.x << "," << player3.y << "," << player.alive << "\0";
        ...
        while( SDL_PollEvent( &event ) )
        {
            ...
            {   
                ...
                switch( event.key.keysym.sym )
                {
                    ...
                    case SDLK_c: screen = SDL_SetVideoMode( SCREEN_WIDTH*drawscale, SCREEN_HEIGHT*drawscale, SCREEN_BPP, SDL_SWSURFACE | SDL_RESIZABLE | SDL_FULLSCREEN );break;
                    case SDLK_v: screen = SDL_SetVideoMode( SCREEN_WIDTH*drawscale, SCREEN_HEIGHT*drawscale, SCREEN_BPP, SDL_SWSURFACE | SDL_RESIZABLE );break;  

                    case SDLK_h: isHost = true; multiplayer = true; break;
                    case SDLK_j: isHost = false; multiplayer = true; draw(); break;
                }
            }
        }
        ...
        while( SDL_PollEvent( &event ) )
        {
            ...
            {   
                switch( event.key.keysym.sym )
                {
                    ...
                    case SDLK_r:if(localmultiplayer){player.set(100,270,64,64,0,0); player.alive = true; player2.set(1430,270,64,64,0,0); player2.alive = true; ball.set(730,500,64,64,0,0);}
                            else if(!multiplayer){player.set(100,270,64,64,0,0); player.alive = true; ai.set(1430,270,64,64,0,0); ai.alive = true; ball.set(730,500,64,64,0,0);} break;
