...
void init() {
  ...
  drawscale =
      (float)desiredScreenWidth /
      SCREEN_WIDTH; // modify to accustom to different window sizes/resolutions
  cout << drawscale;
  ...
  screen = SDL_SetVideoMode(SCREEN_WIDTH * drawscale, SCREEN_HEIGHT * drawscale,
...
void initStage() {
  ...
  for (int i = 0; i < 25; i++) {
    for (int j = 0; j < 15; j++) {
      ...
      ATiles[i][j].setE(i * 64, j * 64, 64, 64, mpc[0], mpc[1], mpc[2], mpc[3],
      ...
      BTiles[i][j].setE(i * 64, j * 64, 64, 64, -1, 0, -1, 0, 1, true, true, 0,
                        0, 0);
...
void draw() {
  if (multiplayer && inTitle) {
      ...
      for (int j = 0; j < 15; j++) {
                                      ...
                                      // or bottom need to be drawn
        {
          if (!ATiles[i - 1][j].needsBorder) {
            apply_surface(drawscale, ATiles[i][j].x, ATiles[i][j].y, Borders,
                          screen, 0, 0, 64, 64);
          }
          if (!ATiles[i + 1][j].needsBorder) {
            apply_surface(drawscale, ATiles[i][j].x, ATiles[i][j].y, Borders,
    ...
    if (multiplayer || localmultiplayer) {
      if (player2.alive) {
        apply_surface(drawscale, player2.x, player2.y, PlayerSprite, screen,
                      &player2.clip_rect);
      } else {
        apply_surface(drawscale, player2.x, player2.y, PlayerDead, screen);
      }
      if (playercount > 2 && player3.alive) {
        apply_surface(drawscale, player3.x, player3.y, PlayerSprite, screen,
                      &player3.clip_rect);
        if (playercount > 3 && player4.alive) {
          apply_surface(drawscale, player4.x, player4.y, PlayerSprite, screen,
                        &player4.clip_rect);
        }
      }
    }
...
void handleMP() // TCP introduction makes both sides crash when moving the
                ...
                // recv failure?)
{
  // This is just to use socketready later and should probably be done once upon
...
*/
  if (isHost) {
    ...
    /*if(playernumber > 2)
    {
        ...
        sendPac->address.port = Client3Adr.port; //port# is wrong! its the TCP
    port number
    ...
    if (SDLNet_TCP_Send(TCPClient, result.c_str(), strlen(result.c_str()) + 1) <
        1) {
...
void update(Uint32 delta) {
  ...
  if (player.alive) {
    ...
    if (BTiles[(player.x + player.w / 2) / 64][(player.y + player.h / 2) / 64]
                .damage > 0 ||
        ATiles[(player.x + player.w / 2) / 64][(player.y + player.h / 2) / 64]
                .damage > 0) {
      player.alive = false;
    }
  }
      ...
      if (event.type == SDL_KEYDOWN) {
        ...
        case SDLK_c:
          screen = SDL_SetVideoMode(
              SCREEN_WIDTH * drawscale, SCREEN_HEIGHT * drawscale, SCREEN_BPP,
              SDL_SWSURFACE | SDL_RESIZABLE | SDL_FULLSCREEN);
          break;
        case SDLK_v:
          screen = SDL_SetVideoMode(SCREEN_WIDTH * drawscale,
                                    SCREEN_HEIGHT * drawscale, SCREEN_BPP,
