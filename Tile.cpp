...
void Tile::update(int deltaT)
{
	if(maxframes > 1)
	{
		...
		if(timeinthisframe >= time2nextframe)
		{
			...
			clip_rect.y = 66 * (framecounter%(maxframes+1)) + framecounter; // 4 tiles per row in tilesheet //67 134 201 268
	        if(clip_rect.y < 20)
	        {
	    	    clip_rect.y = 0;
	        }
...
void Tile::set(int x, int y, int w, int h, int dmgfr, int dmgfrdmg, int img, int maxframes, int time2nextframe, bool traversable, bool active)
		{
			this->x = x;
			this->y = y;
			this->w = w;
...
void EventTile::update(int deltaT)
{
	if(EventActive)
	{
	    ...
	    if(!WaitingForReset) //if we're not done with initial(falling) animation, keep doing that
	    {
	    	if(timeinthisframe >= time2nextframe)
			{
				framecounter++;
	    ...
	    else //if were done with initial animation wait until we want to reset animation
	    {
...
void EventTile::setE(int x, int y, int w, int h, int dmgfr, int dmgfrdmg, int img, int maxframes, int time2nextframe, bool traversable, bool active, int minTime, int maxTime, int TimeTillReset)
{
