...
void EventTile::update(int deltaT) {
  if (EventActive) {
    ...
    if (!WaitingForReset) // if we're not done with initial(falling) animation,
                          // keep doing that
    {
      if (timeinthisframe >= time2nextframe) {
        framecounter++;
    ...
    } else // if were done with initial animation wait until we want to reset
           // animation
    {
...
void EventTile::setE(int x, int y, int w, int h, int dmgfr, int dmgfrdmg,
                     ...
                     int TimeTillReset) {
  set(x, y, w, h, dmgfr, dmgfrdmg, img, maxframes, time2nextframe, traversable,
      active);
