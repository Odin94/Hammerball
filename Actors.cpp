...
void Actor::move(int deltaT, EventTile BTiles[][15], int ballx, int bally, bool lethal)
{
	...
	//x+=velx* ( deltaT / 60.f );//.f interprets it a float; divide by expected MS since last frame (30fps -> /30)
	y+=vely* ( deltaT / 60.f );
    ...
    if(vely < 0)
    {
        ...
        {
            y = BTiles[int(floor((x + w) / 64))][int(y/64)].y + BTiles[int(floor((x + w) / 64))][int(y/64)].h +1;
        }

        if(!BTiles[int(floor(x / 64))][int(y/64)].traversable && y < BTiles[int(floor(x/ 64))][int(y /64)].y + BTiles[int(floor(x/ 64))][int(y /64)].h)
        {
            y = BTiles[int(floor(x / 64))][int(y/64)].y + BTiles[int(floor(x / 64))][int(y/64)].h +1;
        }
    }
    ...
    if(velx < 0)
    {
        ...
        {
            x = BTiles[int(ceil(x / 64))][int((y + h)/64)].x + BTiles[int(ceil(x / 64))][int((y + h)/64)].w;
        }

        if(!BTiles[int(floor(x / 64))][int(y/64)].traversable && x < BTiles[int(floor(x/ 64))][int(y/64)].x + BTiles[int(floor(x/ 64))][int(y/64)].w)
        {
            x = BTiles[int(ceil(x / 64))][int(y/64)].x + BTiles[int(ceil(x / 64))][int(y/64)].w;
        }
    }
    ...
    //time2nextframe = abs(statictime2nextframe * 1/2 *((velx+.5) / (velcapx+1)  +  (vely+.5) / (velcapy+1)));  //+1 to prevent division by 0
...
void Ball::move(int deltaT, EventTile Btiles[][15])
{
	...
	//TODO: Make deceleration affect the vector length; right now the ball moves unnaturally if velx hits 0 while y is still > 0 
	time1+=deltaT; //in ms
	if(time1 >= 100)
	{
...
void AI::setDirection(Ball *ball, Player player)
{
	...
	else
	{
		...
		if(abs(player.x+player.w/2 - bx) <= 120 && abs(player.y+player.h/2 - by) <= 120) 
		{
			velx = ex - px;
			vely = ey - py;
